...
class _AddCameraScreenState extends ConsumerState<AddCameraScreen> {
  ...
  Future<void> _testConnection() async {
    ...
    try {
      ...
      socket.write(_buildRtspOptionsRequest(uri));
      await socket.flush();
      ...
      final statusLine =
          response.isNotEmpty ? response.first : 'Không nhận được phản hồi RTSP';
      final statusCode = _parseRtspStatusCode(statusLine);

      setState(() {
        _testSuccess = statusCode != null && statusCode >= 200 && statusCode < 300;
        _testResult = _buildTestResult(uri, statusCode, statusLine);
      });
    } catch (e) {
      setState(() {
        _testSuccess = false;
  ...
  String _buildRtspOptionsRequest(Uri uri) {
    final path = uri.path.isEmpty ? '/' : uri.path;
    final query = uri.hasQuery ? '?${uri.query}' : '';
    final target = '$path$query';
    final auth = base64Encode(utf8.encode(uri.userInfo));
    return [
      'OPTIONS rtsp://${uri.host}:${uri.port}$target RTSP/1.0',
      'CSeq: 1',
      'User-Agent: IP-Camera-Viewer-Test',
      if (uri.userInfo.isNotEmpty) 'Authorization: Basic $auth',
      '',
      '',
    ].join('\r\n');
  }

  int? _parseRtspStatusCode(String statusLine) {
    final match = RegExp(r'^RTSP/\d.\d\s+(\d{3})').firstMatch(statusLine);
    return match == null ? null : int.tryParse(match.group(1)!);
  }

  String _buildTestResult(Uri uri, int? statusCode, String statusLine) {
    if (statusCode == null) {
      return '✗ Phản hồi RTSP không hợp lệ: $statusLine';
    }

    if (statusCode >= 200 && statusCode < 300) {
      return '✓ Camera phản hồi RTSP hợp lệ tại ${uri.host}:${uri.port}';
    }

    if (statusCode == 401) {
      return '✗ Camera có phản hồi nhưng sai username/password (RTSP 401)';
    }

    if (statusCode == 404) {
      return '✗ Camera có phản hồi nhưng RTSP path không đúng (RTSP 404)';
    }

    return '✗ Camera có phản hồi RTSP nhưng bị từ chối: $statusLine';
  }
